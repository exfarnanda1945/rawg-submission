...
class GamesRepository(private val api: GamesApi) {
    ...
    suspend fun getGameBestOfTheYear(pageSize: Int): Flow<ApiResponse<GameResponseList>> = flow {
        emit(ApiHandler.call { api.getByDatesOrdering(pageSize) })
    }.flowOn(Dispatchers.IO)

    suspend fun getLatestGameRelease(pageSize: Int): Flow<ApiResponse<GameResponseList>> = flow {
        emit(ApiHandler.call { api.getByDatesOrdering(pageSize, ordering = "-released") })
    }.flowOn(Dispatchers.IO)


    suspend fun getList(pageSize: Int): Flow<ApiResponse<GameResponseList>> = flow {
        emit(ApiHandler.call { api.getList(pageSize) })
    }.flowOn(Dispatchers.IO)

    suspend fun search(search: String): Flow<ApiResponse<GameResponseList>> = flow {
